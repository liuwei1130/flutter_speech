...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
  FlutterMethodChannel* channel =
      [FlutterMethodChannel methodChannelWithName:@"speech" binaryMessenger:[registrar messenger]];
...
- (void)activateRecognition:(FlutterResult)result {
  ...
  if (status == SFSpeechRecognizerAuthorizationStatusRestricted ||
      status == SFSpeechRecognizerAuthorizationStatusDenied) {
...
- (void)start:(NSString*)local {
  ...
  _speechRecognitionTask = [speechRecognizer
      recognitionTaskWithRequest:_speechAudioBufferRecognitionRequest
                   resultHandler:^(SFSpeechRecognitionResult* _Nullable result,
                     ...
                     NSLog(@"is final: %d  result: %@", result.isFinal,
...
- (void)recorderSoundStart {
  ...
  if (![_recorder isRecording]) {
    ...
    _levelTimer = [NSTimer scheduledTimerWithTimeInterval:0.5
                                                   target:self
...
- (NSDictionary*)getAudioSetting {
  return [NSDictionary
      dictionaryWithObjectsAndKeys:[NSNumber numberWithFloat:44100.0], AVSampleRateKey,
                                   [NSNumber numberWithInt:kAudioFormatAppleLossless],
                                   AVFormatIDKey, [NSNumber numberWithInt:2], AVNumberOfChannelsKey,
